...
public class BaseDeDatosImplementation extends BaseDeDatos {
  ...
  public int registrarUsuario(Registro registro) {
    int resultado = 0;
    String sql = "INSERT INTO usuarios (correo, nip, nombre, activo, num_intentos) VALUES (?, ?, ?, false, 0)";
  ...
  public int actualizarIntentos(String correo) {
    int resultado = 0;
    String sql = "UPDATE usuarios SET num_intentos = num_intentos + 1 WHERE correo = ?";
    try (PreparedStatement ps = conexion.prepareStatement(sql)) {
      ps.setString(1, correo);
      resultado = ps.executeUpdate();
    } catch (SQLException e) {
  ...
  public int actualizarFechaBloqueo(String correo) {
    int resultado = 0;
    String sql = "UPDATE usuarios SET fecha_bloqueado = CURRENT_TIMESTAMP WHERE correo = ?";
    try (PreparedStatement ps = conexion.prepareStatement(sql)) {
      ps.setString(1, correo);
      resultado = ps.executeUpdate();
    } catch (SQLException e) {
